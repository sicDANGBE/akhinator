...
def decide_next_action(context: dict) -> dict:
    """Variant: choisit la question par Information Gain (entropie)."""
    ...
    if confidence >= 0.82 and step >= 2 and best_id != last_guess_id:
        return {"type": "guess", "item_id": best_id, "confidence": float(confidence)}
    ...
    q = _select_best_question_information_gain(available, candidates, items_by_id)
    if q is None:
        return {"type": "guess", "item_id": best_id, "confidence": float(confidence)}
...
def _select_best_question_information_gain(
    ...
    def H(sz: int) -> float:
        return 0.0 if sz <= 1 else math.log2(sz)
    ...
    for k in feature_keys:
        ...
        # Après answer yes/no, on garde l'inconnu (None) dans les candidats, donc:
        size_yes = t + n
        size_no = f + n
        size_skip = total

        H_after = p_yes * H(size_yes) + p_no * H(size_no) + p_skip * H(size_skip)
        ig = H_before - H_after

        # Petite pénalité si trop d'unknown => questions peu discriminantes
        value = ig - 0.10 * p_skip
        if value > best_value:
            best_value = value
            best_k = k

    # Si le gain est trop faible, mieux vaut guess.
    if best_k is None or best_value <= 0.05:
        return None
    return best_k
